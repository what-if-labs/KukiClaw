# IAQ Reporter MCP - Detailed Usage Guide
...
1. **Use config mode for convenience:**
   ```bash
   mcporter config add iaq-reporter --command "python3 server.py"
...
4. **For scripts, use JSON output:**
   ```bash
   mcporter call iaq-reporter list_devices --output json | jq '.[0].name'
   ```
