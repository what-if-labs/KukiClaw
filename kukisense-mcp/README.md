# IAQ Reporter MCP Server

MCP (Model Context Protocol) server for IAQ Reporter Pro Platform integration.
...
```bash
# Authentication
mcporter call iaq-reporter auth_login email="your-email" password="your-password"
mcporter call iaq-reporter get_current_user

# Buildings & Devices
mcporter call iaq-reporter list_buildings
mcporter call iaq-reporter list_devices
mcporter call iaq-reporter get_device device_id="uuid"

# Sensor Readings
mcporter call iaq-reporter get_latest_readings device_id="uuid"
mcporter call iaq-reporter get_historical_readings \
  device_id="uuid" start_date="2026-05-01" end_date="2026-05-05"

# Batch Operations (faster for multiple devices)
mcporter call iaq-reporter batch_get_latest_readings \
  device_ids='["id1", "id2", "id3"]'

# Alerts & Monitoring
mcporter call iaq-reporter list_alerts status="active"
mcporter call iaq-reporter get_realtime_status
mcporter call iaq-reporter acknowledge_alert alert_id="uuid"

# Compliance
mcporter call iaq-reporter list_standards
mcporter call iaq-reporter calculate_compliance \
  device_id="uuid" standard_id="uuid" start_date="2026-05-01" end_date="2026-05-05"

# Cache Management
mcporter call iaq-reporter cache_stats
mcporter call iaq-reporter cache_clear
```
...
mcporter call --stdio "python3 server.py" \
  iaq-reporter-optimized.get_realtime_status

# Test cache performance
mcporter call --stdio "python3 server.py" \
  iaq-reporter-optimized.cache_stats
```
