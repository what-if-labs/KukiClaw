# Setup Guide - Multiple Ways to Use IAQ Reporter MCP

This guide covers all the ways to connect to and use the IAQ Reporter MCP server.
...
```bash
# Add once
mcporter config add iaq-reporter \
  --command "python3 /path/to/iaq-reporter-mcp/server.py"

# Use anytime
mcporter call iaq-reporter list_devices
mcporter call iaq-reporter get_latest_readings device_id="..."
```

### Option C: JSON output for scripting
```bash
mcporter call iaq-reporter list_devices --output json | jq '.[].name'
```
...
cat > ~/Library/Application\ Support/Claude/claude_desktop_config.json << 'EOF'
{
  "mcpServers": {
    "iaq-reporter": {
      "command": "/path/to/iaq-reporter-mcp/venv/bin/python3",
      "args": ["/path/to/iaq-reporter-mcp/server.py"],
      "env": {
...

{
  "mcpServers": {
    "iaq-reporter": {
      "command": "C:\\path\\to\\iaq-reporter-mcp\\venv\\Scripts\\python.exe",
      "args": ["C:\\path\\to\\iaq-reporter-mcp\\server.py"],
      "env": {
        "IAQ_REPORTER_URL": "https://dashbeta.what-if.sg"
      }
    }
...
```python
import sys
sys.path.insert(0, '/path/to/iaq-reporter-mcp')
...
SERVER_DIR="/path/to/iaq-reporter-mcp"
export IAQ_REPORTER_URL="https://dashbeta.what-if.sg"
...
# 1. Setup
export IAQ_REPORTER_URL="https://dashbeta.what-if.sg"
cd iaq-reporter-mcp && source venv/bin/activate
